...
# katalogiem iCloud) i odpalamy binarkę stamtąd. Kopia bundla w packaging/dist jest
# ostemplowana przez file providera i nie reprezentuje produktu.
set -euo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"

ZIP="$(ls -t "$HERE"/dist/FloorForge-*.zip 2>/dev/null | head -1 || true)"
[ -n "$ZIP" ] || { echo "Brak paczki: $HERE/dist/FloorForge-*.zip — uruchom build_release.sh"; exit 2; }
...
BIN="$(find "$WORK" -type f -path '*/FloorForge.bundle/Contents/Resources/FloorForge/FloorForge' 2>/dev/null | head -1 || true)"
[ -x "${BIN:-}" ] || { echo "SMOKE FAIL: w paczce nie ma FloorForge.bundle/Contents/Resources/FloorForge/FloorForge"; exit 1; }
