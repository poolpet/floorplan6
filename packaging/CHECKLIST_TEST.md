...
Dopóki tu jest PENDING albo FAIL — nie zaczynaj kroków 1–14 i 4b/6b/12b.
...
| # | Krok | Oczekiwane | OK? |
|---|---|---|---|
| 1 | Pobierz zip Safari na konto beta-test, rozpakuj Finderem | folder `FloorForge-<wersja>/` z `FloorForge.bundle`, `INSTALACJA.md`, `INSTALL.md`, `LICENSE-Tapir.txt` i `NOTICE.txt`, bez plików `._*` | |
...
| 3 | **FloorForge → About FloorForge...** | wersja = `<wersja>` z nazwy zipa, port JSON (np. 19723) — NIE `n/a` ani `0` | |
...
| 6b | Otwórz DRUGĄ instancję Archicada (inny projekt), z niej **Room layout** | pasek pokazuje port i projekt **drugiej** instancji (plus `(+1 more)`); eksport domu idzie do niej **bez pytania o instancję** | |
...
| 9 | „Insert into Archicad" | strefy, ściany, drzwi, okna, etykiety w AC; nazwy stref i etykiety po **angielsku** (`Kitchen`, `Living room`, …) — polska nazwa = **FAIL** | |
...
| 12b | Zamknij okno FloorForge i kliknij **Room layout** ponownie **BEZ restartu Archicada** | okno otwiera się znowu; alert „FloorForge is already open" **NIE** pojawia się | |
...
Krok 0 = PASS **i** wszystkie pozycje z kroków 1–14 oraz 4b/6b/12b (razem 17) OK:
