            ...
            "doorsData": {
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "width": { "type": "number", "exclusiveMinimum": 0.0 },
                        "height": { "type": "number", "exclusiveMinimum": 0.0 },
                        "libraryPart": { "type": "string", "description": "Nazwa elementu z biblioteki AC (np. 'Otwór drzwiowy, prostokątny'). Patrz: API_LibPart.docu_UName." },
                        "oSide": { "type": "boolean", "description": "Strona otwierania drzwi. Patrz: API_OpeningBaseType.oSide." },
                        "reflected": { "type": "boolean", "description": "Strona zawiasu (mirror). Patrz: API_OpeningBaseType.reflected." }
                    },
                    "additionalProperties": false,
                    "required": ["ownerWallId", "centerOffset"]
                }
...
GS::ObjectState CreateDoorsCommand::Execute (const GS::ObjectState& parameters, GS::ProcessControl&) const
{
    ...
    return ExecuteCreateWithElements ("Create Doors", [&](GS::Array<GS::ObjectState>& elements) {
        for (const auto& data : doorsData) {
            ...
            // FP6 custom extension: library part lookup po nazwie
            // (port z FP4 PlanWriter.cpp:162-169 1:1)
            GS::UniString libraryPartName;
            if (data.Get ("libraryPart", libraryPartName) && !libraryPartName.IsEmpty ()) {
                API_LibPart libPart = {};
                GS::ucscpy (libPart.docu_UName, libraryPartName.ToUStr ());
                libPart.typeID = APILib_DoorID;
                if (ACAPI_LibraryPart_Search (&libPart, false, true) == NoError) {
                    element.window.openingBase.libInd = libPart.index;
                }
                delete libPart.location;
            }

            // FP6 custom extension: kierunek otwarcia (F8) i strona zawiasu
            bool oSide = false;
