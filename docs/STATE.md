...
>
> **⮕ NEXT SESSION (2026-09-16, FloorForge.bundle — JEDEN dodatek w Dodatkach, UI PO ANGIELSKU: Task 1–7 + fala finalnych fixów DONE, 2 BRAMKI LIVE u Dawida — branch `feat/beta-macos`, NIE pushnięte, tag `v0.7-beta1` nadal ŚWIADOMIE NIE nadany):**
> **CEL WĄTKU:** tester kopiuje **jeden `FloorForge.bundle`** do `/Applications/Graphisoft/Archicad 29/Dodatki/`, startuje AC i klika **FloorForge → Room layout** — okno FloorForge wstaje już połączone z TĄ instancją AC (add-on podaje port JSON w `FLOORFORGE_AC_PORT`). Zero innych czynności instalacyjnych, zero osobnej `.app`, zero osobnego Tapira. Spec `docs/superpowers/specs/2026-09-15-floorforge-bundle-design.md` (§9 = język interfejsu: **angielski**), plan `docs/superpowers/plans/2026-09-15-floorforge-bundle.md`, dziennik `.superpowers/sdd/2026-09-15-floorforge-bundle/`. Spec z 2026-09-14 ma teraz na górze notkę, że **forma dystrybucji jest nieaktualna** (zastąpiona tym specem).
> **DOSTARCZONE (`git log --oneline 8330874..HEAD`):** **T1** `e1c1adb` — `addon/` = fork Tapira 1.4.0 (MIT, `addon/LICENSE` + `addon/NOTICE` z listą zmian) z tożsamością FloorForge (MDID 879970669/416370978, `pl.d7studio.floorforge`, `AddOnName=FloorForge`), przestrzeń komend **`FloorForgeCommand`**, menu Tapira (paleta, Check for Updates, About Tapir) NIE rejestrowane, nowy `FloorForgeLauncher.cpp` spawnujący osadzonego Pythona. **T2** `ada45a9` — `TAPIR_NAMESPACE="FloorForgeCommand"` jedną stałą (bridge + notebooks), `connect()` bierze port z `FLOORFORGE_AC_PORT` bez skanu (martwy port → fallback na skan + log). **T3** `ff77adc` — `packaging/build_release.sh`: cmake + PyInstaller onedir do `Contents/Resources/FloorForge/` + ad-hoc codesign `--deep` + zip + **bramka NA ZIPIE** (AppleDouble, codesign, Mach-O, architektura, `Info.plist`, `--selftest`). **T5** `f609192`+`d491a88` — `install_local.sh` (nowy bundle składany obok celu jako `.FloorForge.new`, podmiana dopiero po udanym podpisie), `INSTALACJA.md`, `CHECKLIST_TEST.md`. **T3b** `fea727b`+`9c38cf5`+`653e561` — pasek AC odświeża się sam przy starcie z add-onu, logo FloorForge zamiast Tapirowego, twardy błąd gdy AC nie oddaje portu JSON, **dodatek arm64-only** + bramka architektury. **T5b** `3b07e44` — kopie starych dodatków lądują **POZA Dodatkami** (`~/Library/Application Support/FloorForge/backups/<ts>`). **T7 (angielskie UI)** `0e45545`+`c8d2a4f`+`87c588c`+`b1757ee` — §9 specu, całe GUI/komunikaty/`user_errors` po angielsku, zasoby dodatku po angielsku („Room layout", „About FloorForge...", alerty launchera), `INSTALL.md` (EN) w zipie obok `INSTALACJA.md` (PL). **FALA FINALNYCH FIXÓW po review całej gałęzi (2026-09-16):** `3a39ab5` — launcher czeka **700 ms** po spawnie i sprawdza `IsTerminated()`: proces ubity przez Gatekeepera daje teraz alert o kwarantannie zamiast menu-które-nic-nie-robi (+ `AboutDialog` nie pokazuje „JSON port: 0", NOTICE/README z nazwami menu jak w AC); `e616718` — **`bridge/room_names.py`** (`ROOM_NAMES_EN` + `room_name_en()`) wpięte na szwach: nazwa strefy w AC, tekst etykiety (składany z `LabelSegment.room_name`, powierzchnia nietknięta), tytuł rzutu, etykiety pokoi, panele szczegółów, podpisy paneli (`GROUND FLOOR`/`ATTIC`/`FIRST FLOOR`), legenda (`Furniture`, `Entrance door`), komunikaty błędów w `plan_writer`/`house_writer`/`boundary_reader` — **`core/` NIETKNIĘTE**, klucze `parter`/`poddasze` bez zmian; `19b1fcc` — `tests/test_no_polish_user_strings.py` (skan AST 10 modułów, bez docstringów i argumentów `logger.*`/`print`) + `tests/test_room_names.py` (każda `nazwa` z `templates/*.json` + szew eksportu na fake'owym Tapirze); `060b0a7` — **port z env wygrywa**: `AcStatusWidget` przesuwa instancję z `FLOORFORGE_AC_PORT` na początek listy, eksport domu bierze ją bez pickera (spec §4); `49f523d` — `LICENSE-Tapir.txt`+`NOTICE.txt` w zipie, bramka `lipo` na **RÓWNOŚĆ** z `arm64` (nie na obecność), `install_local.sh` z sufiksem `-$$` i wynoszeniem `.floorforge-backup-*` PRZED przeniesieniem działającego dodatku, `INSTALL`/`INSTALACJA` z sekcją Troubleshooting (brak menu / kwarantanna `xattr -dr com.apple.quarantine` / gdzie log), checklista + wiersze **6b** (dwie instancje AC) i **12b** (ponowne otwarcie okna bez restartu AC).
> **ZNALEZISKO LIVE (2026-09-15, Dawid):** AC zgłosił „Niektóre dodatki nie mogły być załadowane" — Menedżer dodatków pokazał, że **Archicad skanuje także KROPKO-KATALOGI w Dodatkach** i wczytywał kopie zapasowe (`.floorforge-backup-*/FloorForge.bundle` „już załadowany", stary FloorPlan4 z tym samym MDID, stary Tapir). Główny `FloorForge.bundle` ładował się poprawnie. Stąd `3b07e44` (backupy poza Dodatki) i zdanie o ukrytych podfolderach w obu instrukcjach.
> **DECYZJE (Dawid):** (1) **arm64-only z premedytacją** — osadzony Python jest arm64-only, więc universal dodatek załadowałby się na Intelu i dopiero spawn by padł; AC na Intelu po prostu nie ładuje dodatku. (2) **`codesign --deep` + kod wykonywalny w `Contents/Resources/` zamykają drogę do notaryzacji** — Apple odrzuca oba; dla bety ad-hoc wystarcza, ale wyjście poza krąg znajomych będzie wymagało przepakowania (Python do `Contents/MacOS/` albo osobny helper-bundle) i podpisu Developer ID. (3) **Interfejs po angielsku** (spec §9) — testerzy spoza Polski; dokumentacja PL+EN, log techniczny może być mieszany. (4) **Po becie „poziom 1"** = natywna paleta w AC (reuse `FloorPlan4_CPP` `FloorPlanPalette`/`PlanWriter`/`BoundaryReader`) + **headless serwis Pythona** (`service/` HTTP) w bundlu; pełny mózg w C++ („poziom 2") odłożony jako decyzja dystrybucyjna.
> **🟡 BRAMKI LIVE (Dawid, obie PENDING):** (1) **spike kwarantanny (Task 4 planu)** — pobrać zip Safari na konto beta-test, sprawdzić `xattr -p com.apple.quarantine` na zipie i na rozpakowanym bundlu, zainstalować, kliknąć Room layout; wynik wpisać do `packaging/CHECKLIST_TEST.md` (Krok 0) jako PASS/FAIL + data. Alert „FloorForge started but exited immediately…" z `3a39ab5` jest teraz **oczekiwanym** zachowaniem przy FAIL. (2) **`packaging/CHECKLIST_TEST.md`** na czystym koncie — kroki 1–14 + 4b/6b/12b (razem 17). Dopiero komplet OK → tag `v0.7-beta1` i wysyłka zipa.
> **ZNANE LUKI (świadome, NIE regresje):** (1) **beta-GUI nigdy nie przeklikane end-to-end przez automat** — tylko `--selftest` + testy modułowe; realne klikanie = bramka #2. (2) **Brak notaryzacji** (patrz decyzja 2) → bundle pobrany z internetu może wymagać `xattr -dr` (udokumentowane w Troubleshooting). (3) **GUI liczy in-process** — serwis HTTP jest pod przyszłe powłoki, `/export` zwraca 501. (4) **Polskie nazwy zostają w `core/` i `templates/*.json`** (są identyfikatorami solvera) — tłumaczy je `bridge/room_names.py` na szwach; nazwa dodana w `core/` bez wpisu w słowniku przejdzie **bez tłumaczenia** (bramką jest `tests/test_room_names.py`). (5) **iCloud file provider re-stempluje FinderInfo** → `codesign` weryfikować z zipa, nie z `packaging/dist/`. (6) Drzwi poddasza (template `sasiedztwo` vs room-set) — znany MVP-gap, nietknięty.
> **NEXT:** bramki live → wpis tutaj → tag → wysyłka. `git push` i FF do `main` = decyzja Dawida (gałąź NIE pushnięta w tej sesji).
>
> **Previously — (2026-09-14, BETA DYSTRYBUCYJNA macOS/AC29: Task 0–8 DONE, 2 BRAMKI LIVE u Dawida — branch `feat/beta-macos` PUSHNIĘTY, tag `v0.7-beta1` ŚWIADOMIE NIE nadany):**
> **CEL WĄTKU:** zamrożona `.app` dla 2–3 testerów bez Pythona (macOS + AC 29), polskie komunikaty, log na dysku, dom jednym klikiem na obie kondygnacje. Spec `docs/superpowers/specs/2026-09-14-beta-dystrybucja-macos-design.md`, plan `docs/superpowers/plans/2026-09-14-beta-dystrybucja-macos.md`, dziennik `.superpowers/sdd/2026-09-14-beta-dystrybucja-macos/` (`progress.md` + `task-*-report.md`).
> **DOSTARCZONE (Task 0–8, każdy z review na Opusie + rundami fixów):** **T0** higiena repo + tag `v0.6-baseline` pushnięty na `main` (`0f8a142`; pełna suita **585 passed / 26 skip / 0 fail w 68 min**) + controller-chore `.gitignore` na duplikaty `* 2.*` odtwarzane przez iCloud (`305c9bc`). **T1 — dom→AC jednym klikiem:** `TapirConnection.activate_story()` + `story_navitems()` + lista `CHANGE_WINDOW_SHAPES` (4 kandydaci na kształt parametru `ChangeWindow`) + GUI checkbox „Wstaw obie kondygnacje (auto-przełączanie w AC)" (`bd99170`); fix I1/I2 — klucze story w **PRZESTRZENI INDEKSÓW AC** (`firstStory`, nie 0..n) + **zero częściowego eksportu** gdy przełączenie padnie w połowie (`b51e75e`); fix piwnicy — parter = indeks **0** gdy `first<=0<=last`, inaczej `first` (`6e312cf`); 38 testów. **T2–T4 — serwis HTTP** (pod przyszłe powłoki; GUI go NIE używa): `service/solve_adapter` JSON→kontrakt z polskimi `ValueError` (`2c1b6f4`, `1d8d524`), `JobStore` — joby z postępem i statusem (`ea03913`), serwer `health/solve/jobs/export` + klient (`83ae4e2`) + hardening: catch-all 500 po polsku, sanity `Content-Length`, query string (`a42a570`). **T5 — UX błędów:** `ui/user_errors.describe(exc)->(tytuł, tekst)` po polsku + `ui/app_logging` (`~/Library/Logs/FloorForge/floorforge.log`, rotacja 5×2 MB, ścieżka logu pokazywana w oknie błędu) (`f588b08`). **T6 — tryb beta:** `FLOORFORGE_BETA=1` → jedna zakładka „Podział rzutu" + `AcStatusWidget` (pasek `AC port … · projekt · kondygnacja` + „Odśwież") (`e046bb2`), fix: polskie etykiety po resetach, status AC nie przestawia portu eksportu, koniec ucinania tekstu (`142908f`); **T6b** lewy panel w `QScrollArea` — czytelny na 13" przy 1100×700 (`ca75593`, `58041e4`). **T7 — entry point** `floorforge_app.py` + `--selftest` (solve przez serwis, bez okna i bez AC; łapie też awarię startu serwisu) (`2bb12fa`, `e037e07`). **T8 — pakowanie:** PyInstaller spec + `packaging/build_release.sh` + `smoke_frozen.sh` + `INSTALACJA.md` + ikona + `requirements-lock.txt` (`6ab9489`), bramka release przeniesiona **NA PACZKĘ**, nie na kopię z iCloud (`8553243`) → artefakt **`packaging/dist/FloorForge-beta-v0.6-baseline-18-g8553243.zip`, 80 MB**, SMOKE OK z zipa, `codesign --verify --deep --strict` (ad-hoc) PASS z zipa. **T9:** `packaging/CHECKLIST_TEST.md` + ten wpis + sekcja „Beta" w `README.md`. **T10 — fala finalnych fixów po review CAŁEJ gałęzi:** `parter_story_index(first,last)` jako JEDNO źródło prawdy dla `check_active_story` i ścieżki eksportu w GUI (przy piwnicy `firstStory=-1` guard kierował testera na piwnicę zamiast na parter); autouse fixture `_isolated_env` kasuje `FLOORFORGE_BETA` (`floorforge_app.main()` ustawiał je na PRAWDZIWYM środowisku → każdy test po `test_selftest.py` leciał w becie); pusty podgląd cytuje realne etykiety przycisków (`_placeholder_text`); CI z `QT_QPA_PLATFORM=offscreen`; `build_release.sh` — `ditto --norsrc --noextattr` (zero `._*` w zipie, sprawdzane bramką) + `packaging/dist` ruszany DOPIERO po przejściu bramki (nieudany build nie kasuje ostatniej dobrej paczki), katalog roboczy zostaje przy porażce; `PyYAML>=6.0` w `requirements.txt`; docstring `ui/main_window.py`, README (status etapów + realne liczby testów), `INSTALACJA.md` (stan „ArchiCAD: nie sprawdzono"), `CHECKLIST_TEST.md` (krok 1, krok 10B = ręczne 2 przebiegi + trzeci wynik = FAIL, Gatekeeper w kroku 13). **Pełna suita po T10: 668 passed / 26 skipped / 1 xpassed / 1 failed w 70:06** — jedyny fail to `tests/test_house_service_placement.py::test_two_storey_kotlownia_parter_garderoba_pietro_external` (CP-SAT z `time_limit_s=60`, wynik zależny od obciążenia maszyny; **w izolacji przechodzi**, `core/` nietknięte). Paczka: `FloorForge-beta-v0.6-baseline-22-g835e5c1.zip`, 83 494 399 B, zero AppleDouble, codesign z zipa PASS, SMOKE OK.
> **✅ BRAMKA LIVE #1 ZALICZONA 2026-09-15:** Dawid odpalił probe na żywym AC → `ZWYCIĘZCA: navigatorItemId:{guid}` = kształt #1 na liście `CHANGE_WINDOW_SHAPES` (`bridge/tapir_connection.py:174`) → kolejność bez zmian, paczka bez rebuildu. (Offline `{navigatorItemId:{guid}}` rzucał schema-reject — na żywym AC29 działa.) Pozostaje BRAMKA #2 (checklist). Stary opis bramki #1:
> **🟡 BRAMKA LIVE #1 (Dawid, AC z projektem 2-kondygnacyjnym, bezczynne):** `PYTHONPATH=. python notebooks/ac_story_switch_probe.py` → **wklej linię `ZWYCIĘZCA: …` tutaj** → zwycięski kształt **przesuń na początek `CHANGE_WINDOW_SHAPES` w `bridge/tapir_connection.py`** i przebuduj paczkę. `ZWYCIĘZCA: BRAK` = auto-switch nieosiągalny na tym AC → beta idzie z ręcznym przełączaniem (checkbox off, 2 przebiegi), a ostrzeżenie „Nie udało się automatycznie przełączyć…" + ZERO wstawionych elementów jest wtedy **poprawnym** zachowaniem.
> **🟡 BRAMKA LIVE #2 (Dawid, czyste konto macOS):** `packaging/CHECKLIST_TEST.md`, 13 kroków (14 przy wariancie B — krok 10B) (+ krok 0 = bramka #1). Dopiero **wszystkie OK → tag `v0.7-beta1`** (celowo NIE nadany wcześniej — tag ma oznaczać paczkę, która przeszła bramkę) i wysyłka zipa 2–3 osobom z prośbą „przy błędzie prześlij log + zrzut". Dowolny FAIL → wpis tutaj, poprawka, rebuild, powtórka od kroku 1.
> **ZNANE LUKI (świadome, NIE regresje):** (1) **GUI liczy in-process** — serwis HTTP jest pod przyszłe powłoki, `/export` zwraca **501** (eksport do AC tylko z GUI); (2) **beta-GUI nigdy nie przeklikane end-to-end przez automat** — tylko `--selftest` (solve headless) + testy modułowe; realne klikanie = bramka #2; (3) **iCloud file provider re-stempluje FinderInfo** → `codesign` weryfikować **z zipa**, nie z `packaging/dist/`; (4) w becie zostały **angielskie nagłówki sekcji** (`1. Outline`, `2. Type and options`, `4. Result`) i resztki angielskich stringów — przyciski i komunikaty są po polsku. *(Luki „`check_active_story` mapuje parter↔`firstStory`" i „CI bez `QT_QPA_PLATFORM=offscreen`" — ZAMKNIĘTE w T10.)*
> **NEXT po bramkach:** feedback testerów → tutaj; fast-forward `feat/beta-macos` → `main` to decyzja Dawida (branch pushnięty na origin, `main` nietknięty poza tagiem `v0.6-baseline`).
>
> **Previously — (2026-06-22, DOM→AC: target instancji + story-guard WDROŻONE, auto-switch do nailingu live — branch `feat/sfh-open-plan-day-zone` ZMERGOWANY do `main` FF, NIE pushnięte; `main` 140 przed `origin/main`):**
> **RUCH ZERO:** gałąź zmergowana do `main` (clean fast-forward), working tree posprzątany (STATE/spec/sondy scommitowane), `rzuty/` → `.gitignore` (19MB wzorców D7 lokalnie). **NIE pushnięte** (push = na żądanie; do realnego de-riska trzeba `git push`).
> **LIVE-RESEARCH API Tapira (read-only, nasz stack + MCP — KAŻDY fakt zweryfikowany na żywym AC, zero zgadywania):** (a) Tapir `GetProjectInfo`→`projectName/projectPath` (standardowe NIE istnieje w naszym wrapperze — używamy Tapirowego); (b) Tapir `GetStories`→`actStory`(indeks aktywnej)+`firstStory/lastStory`+`stories[]`; (c) **`GetElementsByType("Wall")` przez nasz stack zwraca 73/37 NIE 0** → schemat ma `filters`(OnActualFloor)+`databases` default null, scope = **aktywna baza planu**; „0 okien poddasza" było STANEM aktywnego okna/story, nie zepsutym call'em; (d) ProjectMap `GetNavigatorItemTree`→`StoryItem`y (top-down → `reversed`=idx rosnące, Parter=idx0); (e) story-target głębszy: `ChangeWindow`(nav-item) / `SetDetailsOfElements.floorIndex` istnieją.
> **WDROŻONE (TDD, mózg+mieszkania NIETKNIĘTE):** **Task 1** `bridge/tapir_connection.py`: `list_instances()` (port+nazwa via GetProjectInfo, świeże połączenia bez mutacji singletona) + `use_port()` (jawny port, bez prefer-selection) + `get_stories()`/`get_project_info()` + `check_active_story()` (czysta logika, mapowanie po INDEKSIE: parter↔first, poddasze↔>first, 1-kond.+poddasze=blok) + log gdy `get_all_walls`=0 (commit `011082b`, `tests/test_ac_targeting.py` 13/13). **Task 2** GUI `_export_house_to_archicad`: picker `QInputDialog` gdy >1 instancja (port+nazwa, pamięta wybór `_ac_target_port`) + story-guard (twarde ostrzeżenie z furtką „wstaw mimo to") + eksport do JEDNEJ wybranej instancji (`use_port`+`tapir=`), status z portem+nazwą (commit `8ab2518`, `tests/test_house_export_gui.py` 5/5). **Task 4** diag `notebooks/ac_story_diag.py`: projectName+actStory per port NAJPIERW (widoczne gdy AC zajęty), jawna linia DOM-* z portem — **zweryfikowany live** (Kamienica 19723: actStory=0, stories Parter/Poddasze/'', Zone18/Wall37) (commit `a08659f`).
> **TASK 3 — AUTO-SWITCH (1-klik obie kondygnacje, DECYZJA DAWIDA) — NIE DOKOŃCZONE, blokada = live:** mechanizm = `ChangeWindow` do story nav-item PRZED tworzeniem (active story=piętro → cały writer + okna `GetElementsByType` active-DB lądują dobrze; programowa wersja sprawdzonego manualnego 2-pass). **BLOKER: `{navigatorItemId:{guid}}` rzuca schema-reject `additionalProperties #/navigatorItemId`** (nasz stack I MCP) → param FINICKY, trzeba namierzyć na żywym AC; „create-lands-on-target" niewryfikowalne offline (mutacja realnego projektu Dawida). **Probe gotowy `notebooks/ac_story_switch_probe.py`** (próbuje 5 kształtów ChangeWindow → który zmienia actStory, RESTORE) — **Dawid odpala**. Alt jeśli ChangeWindow nie do okiełznania: `SetDetailsOfElements.floorIndex` retarget po eksporcie.
> **OTWARTE PYTANIE okna (B5, live):** parter dostał okna, poddasze 0 — host okien (czy poddasze ma ściany obrysu do podpięcia?). Mine #2 prawdopodobnie domknie się gdy aktywna story=docelowa. `get_all_walls` loguje teraz gdy 0.
> **🟡 PENDING LIVE DAWID (bramka):** (1) **live-test MANUAL 2-pass z picker+guard** → CEL SESJI (dom do JEDNEJ znanej instancji, parter+poddasze na właściwych story, `ac_story_diag` potwierdza DOM-PARTER-* na story0 + DOM-PODDASZE-* na story1, ten sam port); (2) **odpal `ac_story_switch_probe`** (AC bezczynny!) → wklej output → zbuduję `activate_story` + 1-klik. Drzwi poddasza = znany MVP-gap (template `sasiedztwo` vs room-set), NIE ruszane.
> **PO SESJI (bramka decyzyjna):** przepuść 1 realny rzut przez pełny pipeline (poligon→generate→eksport) i zmierz czy szybciej niż ręcznie. Spec/plan: `docs/superpowers/{specs,plans}/2026-06-22-dom-ac-story-target-multi-instance*`. Pamięć [[project_ac_multi_instance_hazard]] [[project_ac_export_apartment_confirmed]].
>
> **Previously — (2026-06-19, DOM→AC LIVE-DEBUG: „poddasze nie wchodzi" — branch `feat/sfh-open-plan-day-zone`, NIE pushnięte):**
