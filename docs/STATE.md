...
> **🟡 BRAMKA LIVE #2 (Dawid, czyste konto macOS):** `packaging/CHECKLIST_TEST.md`, 13 kroków (+ krok 0 = bramka #1). Dopiero **13/13 OK → tag `v0.7-beta1`** (celowo NIE nadany wcześniej — tag ma oznaczać paczkę, która przeszła bramkę) i wysyłka zipa 2–3 osobom z prośbą „przy błędzie prześlij log + zrzut". Dowolny FAIL → wpis tutaj, poprawka, rebuild, powtórka od kroku 1.
> **ZNANE LUKI (świadome, NIE regresje):** (1) **GUI liczy in-process** — serwis HTTP jest pod przyszłe powłoki, `/export` zwraca **501** (eksport do AC tylko z GUI); (2) **beta-GUI nigdy nie przeklikane end-to-end przez automat** — tylko `--selftest` (solve headless) + testy modułowe; realne klikanie = bramka #2; (3) **iCloud file provider re-stempluje FinderInfo** → `codesign` weryfikować **z zipa**, nie z `packaging/dist/`; (4) `check_active_story` wciąż mapuje parter↔`firstStory` (doradcze ostrzeżenie z furtką „wstaw mimo to", nie blokuje) — niespójne z fixem piwnicy w ścieżce eksportu; (5) w becie zostały **angielskie nagłówki sekcji** (`1. Outline`, `2. Type and options`, `4. Result`) i resztki angielskich stringów — przyciski i komunikaty są po polsku; (6) **CI `.github/workflows/test.yml` nie ustawia `QT_QPA_PLATFORM=offscreen`** → moduły testów GUI inne niż `test_gui.py` mogą paść headless na GitHub Actions.
